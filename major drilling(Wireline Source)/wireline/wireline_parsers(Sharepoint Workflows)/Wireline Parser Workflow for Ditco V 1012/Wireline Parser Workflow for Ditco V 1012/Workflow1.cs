...
namespace wireline_parser_new_format
{
    public sealed partial class Workflow1 : SequentialWorkflowActivity
    {
        ...
        private void codeActivity1_ExecuteCode(object sender, EventArgs e)
        {
            ...
            //SqlConnection dbConn = new SqlConnection("Data Source=BSIMONCTONDAMIA;initial catalog=wireline;User ID=wirelinetest;Password=wirelinetest;");
            //SqlConnection dbConn = new SqlConnection("Data Source=BSISISECURE01;initial catalog=wireline;User ID=wireline_user;Password=w1reline;");
            ...
            while((line = sr.ReadLine()) != null)
            {
                ...
                string firstSet =
                    ...
                    + arrayString[20] + ","					//Start Date				(Ditco)
                    + arrayString[21];                      //Start Point               (Ditco)

                string thirdSet = 
                      arrayString[arraySize - 4] + ","  //pk frc
                    + arrayString[arraySize - 3] + ","  //pk dep
                    + arrayString[arraySize - 2] + ","  //distance travelled
                    + arrayString[arraySize - 1];       //end time

                wireline_rate = int.Parse(arrayString[5]);
	            wireline_direction = arrayString[19];
	            string wireline_measurement_unit = arrayString[18];

	            //Check to make sure there is actually at least 1 speed and 1 force and 1 depth in the record. If there is not, default them to zero.
                string compareString = arrayString[22] + "," + arrayString[23] + "," + arrayString[24] + "," + arrayString[25];

                if (thirdSet.CompareTo(compareString) == 0)
                { //zero means equal
                    ...
                    finalrecord = firstSet + ",0,0,0" + thirdSet + "," + run_no.ToString();
                }
                else
                {
                    for (int b = 22; b < arraySize - 4; b = b + 3)
                    {
                        ...
                        /* WE DON'T NEED THIS VALUE IN THE NEW FORMAT - SEPT. 2008
                        if (b != 22)
                        {
                        ...
                        */
                        ...
                        finalrecord = firstSet + "," + arrayString[b] + "," + arrayString[b + 1] + "," + arrayString[b + 2] + "," + thirdSet + "," + run_no.ToString();
                        
                        String[] arrayStringFinal = finalrecord.Split(',');
                        int param_count = arrayStringFinal.Length;

                        SqlCommand InsertCMD = new SqlCommand("wireline_load_new_format", dbConn);
                        SqlCommand postProc = new SqlCommand("post_load_processing_new_format", dbConn);
                        ...
                        */
                        ...
                        param = InsertCMD.Parameters.Add("@start_point", SqlDbType.Char);
                        param.Value = arrayStringFinal[21];
                        param.Direction = ParameterDirection.Input;
                        ...
                        param = InsertCMD.Parameters.Add("@real_depth", SqlDbType.Char);
